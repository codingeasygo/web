...
// 	if a.ShowLog {
// 		DebugLog("AttrFilter do filter to path(%v) with token(%v),attrs(%v)", hs.R.URL.Path, token, util.S2Json(attrs))
// 	}
...
// 	if a.ShowLog {
// 		DebugLog("AttrFilter(Web) register token(%v) by args(%v) success", token, util.S2Json(args))
// 	}
...
// 	a.TimeL[token] = util.Now()
// 	if a.ShowLog {
// 		DebugLog("AttrFilter(RC) register token(%v) by args(%v) success", token, util.S2Json(rc.Map))
// 	}
// 	return token, nil
// }
...
// 	}
// 	if removed > 0 {
// 		DebugLog("AttrFilter do timeout with %v token be removed", removed)
// 	}
// }

// func (a *AttrFilter) loop_timeout(delay int64) {
// 	a.running = true
// 	DebugLog("AttrFilter start loop timeout with delay(%v),timeout(%v)", delay, a.Timeout)
// 	for a.running {
// 		a.ChkTimeout()
// 		time.Sleep(time.Duration(delay) * time.Millisecond)
// 	}
// 	DebugLog("AttrFilter loop timeout is stopped")
// }
