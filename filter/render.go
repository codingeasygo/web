...
// 	DebugLog("Render doing %v", hs.R.URL.Path)
// 	if hs.RVal("_data_") == "1" {
...
// 		if err != nil {
// 			ErrorLog("Render store cache data fail with %v", err)
// 		}
// 	} else {
// 		ErrorLog("Render prepare response data fail with %v", err)
// 		cache, lerr := r.loadCacheData(hs)
// 		if lerr == nil && len(cache) > 0 {
// 			ErrorLog("Render prepare response data fail with %v, and using cache(%v)", err, len(cache))
// 			hs.W.Write(cache)
// 		} else {
// 			ErrorLog("Render prepare response data fail with %v, and load cache fail with len(%v),%v", err, len(cache), lerr)
...
// 			DebugLog("Render read cache from %v success", cacheFile)
// 		}
// 	}
// 	return
// }
...
// 	if len(r.CacheDir) > 0 {
// 		cacheFile := filepath.Join(r.CacheDir, r.cacheFilename(hs))
// 		DebugLog("Redner saving cache to file %v", cacheFile)
